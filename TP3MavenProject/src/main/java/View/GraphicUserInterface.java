...
/**
 * Class GraphicUserInterface Interface utilisateur
 * 
...
public class GraphicUserInterface implements Observer {
	...
	private void initialize() {
		...
		button = new JButton[model.getListeCommande().size()];
		...
		for (Integer i = 0; i < button.length; i++) {
			...
			nom = model.getListeCommande().get(i).getNom();// "Commande " +
															// (i).toString();
	...
	public void update(Observable arg0, Object arg1) {
		...
		case "Update Commands":
			...
			for (Integer i = 0; i < affichages.length; i++) {
				button[i].setEnabled(model.getCommandeValide(i));
				affichages[i].setEnabled(model.getCommandeValide(i));
				affichages[i].setText(model.getResultatsCommande(i));
			}
			break;

		case "Update Tree":
			scrollPane.setViewportView(model.getArbre());
			scrollPane.repaint();
			break;
